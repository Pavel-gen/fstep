...
module.exports = {
    ...
    deleteNote: async (parent, {id}, {models, user}) => {
        if (!user){
            throw AuthenticationError('You must be signed to delete note')
        }
    ...
    updateNote: async (parent, {content, id}, {models}) => {
        if (!user){
            throw AuthenticationError('You have to be signed to update this note')
        }
        const note = await models.Note.findById(id)
        if (note && String(note.author) !== user.id){
            throw ForbiddenError("You don't have permissions to update the note.")
        }
        return await models.Note.findOneAndUpdata(
            {
                _id: id
            },
            {
